import { renderHook, act, WaitForNextUpdate, RenderResult } from '@testing-library/react-hooks/dom'
import { TQuestionGeneration, useQuestionGeneration } from './usequestiongeneration'
...
describe("useQuestionGeneration", () => {
    ...
    describe("newRoot", () => {
        it("Should generate a new Root", () => {
            ...
            expect(result.current.question).toStrictEqual([{ "abc": "f", "midi": 77, "name": "F", "note": "F5", "octave": 5}])
    ...
    describe("newChord", () => {
        ...
        it("Should generate a new Chord", () => {
            ...
            expect(result.current.question.map(note=>note?.note)).toStrictEqual(["F5", "A5", "Cb6"])
            expect(result.current.current).toBe(0)
        ...
        it("Should go to the next note in the chord", () => {
            ...
            expect(result.current.question.map(note=>note?.note)).toStrictEqual(["C6", "Eb6", "F6", "G6"])
            expect(result.current.current).toBe(0)
        })
    })
    describe("newInterval", () => {
        ...
        it("Should generate a new Interval", () => {
            ...
            expect(result.current.question.map(note=>note?.note)).toStrictEqual(["F5", "G5"])
            expect(result.current.current).toBe(0)
        ...
        it("Should go to the next note in the interval", () => {
            ...
            expect(result.current.question.map(note=>note?.note)).toStrictEqual(["E6", "G6"])
            expect(result.current.current).toBe(0)
        })
    })
})