...
import { useFormState } from "../../util/customhooks/customhooks";
import {
	DirectionType,
	directionTypes,
	intervals,
...
export function Interval({
	...
	newInterval: (intervalRange: number, direction: DirectionType) => void;
	root: IStdNote;
}) {
		...
		<>
			{root ? (
				<>
					<Select
						label={"Direction"}
						array={directionTypes}
						id={"direction"}
						cb={function (e: ChangeEvent<HTMLSelectElement>): void {
							let target = e.target;
					...
					<Slider
						...
						label={"Intervals"}
						value={intervalRange}
						set={setIntervalRange}
						min={0}
						max={12}
						display={intervals[parseInt(intervalRange)]}
					/>
					<button
						onClick={() => {
							newInterval(parseInt(intervalRange), direction);
						}}
					>
						New Interval
					</button>
