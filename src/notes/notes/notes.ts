...
export function StdNote(name: NoteLiteral): IStdNote {
	...
	if (note.empty || !note.oct || !note.midi) {
		...
		return {
			...
			name: note.pc,
			octave: note.oct,
			abc: AbcNotation.scientificToAbcNotation(note.name),
			midi: note.midi,
		};
	}
}
...
export function createInterval(
	...
	if (random != null && root) {
		...
		if (note) {
			...
			return {
				note: normalizeMidi(note, randomFromArray([true, false])),
				description,
			};
		}
	}
...
export function randomChord(difficulty: number, root: string) {
	let filter = ChordType.symbols().filter((e) => {
		return Chord.get(e).intervals.length == difficulty;
	});
