...
describe("StdNote: Creates a standard note format", () => {
	...
	it("Should return a full value", () => {
		expect(Notes.StdNote("c4")).toEqual({
			note: "C4",
			name: "C",
			octave: 4,
			abc: "C",
...
describe("FromFreq: Creates a standard note from frequency", () => {
	it("Should return a StdNote", () => {
		expect(Notes.FromFreq(440)).toEqual({
			note: "A4",
			name: "A",
			octave: 4,
			abc: "A",
...
describe("generateRandomNote: Returns a random StdNote", () => {
	...
	it("Should return a StdNote", () => {
		...
		expect(Notes.generateRandomNote(10, 30, false)).toEqual({
			abc: "E",
			midi: 64,
			name: "E",
			note: "E4",
	...
	it("Should return a Sharp", () => {
		jest.spyOn(global.Math, "random").mockReturnValue(0.3);
		expect(Notes.generateRandomNote(10, 30, true)).toEqual({
			abc: "^E",
			midi: 65,
			name: "E#",
			note: "E#4",
...
describe("normalizeMidi: returns a StdNote", () => {
	it("Should return a sharp StdNote if you tell it to", () => {
		expect(Notes.normalizeMidi(61, true)).toEqual({
			note: "C#4",
			name: "C#",
			octave: 4,
			abc: "^C",
	...
	it("Should return a flat StdNote if you tell it to", () => {
		expect(Notes.normalizeMidi(61, false)).toEqual({
			note: "Db4",
			name: "Db",
			octave: 4,
			abc: "_D",
...
describe("normalizeNote: returns a StdNote", () => {
	it("Should return a StdNote", () => {
		expect(Notes.normalizeNote({ natural: "C" }, 4, true)).toEqual({
			note: "C4",
			name: "C",
			octave: 4,
			abc: "C",
	...
	it("Should return a sharp StdNote if you tell it to", () => {
		expect(Notes.normalizeNote({ sharp: "C#", flat: "Db" }, 4, true)).toEqual({
			note: "C#4",
			name: "C#",
			octave: 4,
			abc: "^C",
	...
	it("Should return a flat StdNote if you tell it to", () => {
		expect(Notes.normalizeNote({ sharp: "C#", flat: "Db" }, 4, false)).toEqual({
			note: "Db4",
			name: "Db",
			octave: 4,
			abc: "_D",
...
describe("createInterval: returns an IInt type", () => {
	...
	it("Should generate an Ascending Major Second", () => {
		...
		expect(
			Notes.createInterval(Notes.StdNote("C4"), 4, "Ascending")
		).toStrictEqual({
	...
	it("Should generate an Descending Perfect Fourth", () => {
		jest.spyOn(global.Math, "random").mockReturnValue(0.4);
		expect(
			Notes.createInterval(Notes.StdNote("C4"), 4, "Descending")
		).toStrictEqual({
...
describe("matchScales: this will take an array of notes and match them to available scales taking into account the selected key", () => {
	it("Should return an array of scales", () => {
		expect(
			Notes.matchScales(["C", "D", "E", "F", "G", "A", "B"], "F")
		).toStrictEqual(["lydian", "ichikosucho", "chromatic"]);
	});
});
...
describe("getScaleNotes: this will return notes from a scale", () => {
	it("Should return an array of notes", () => {
		expect(Notes.getScaleNotes("C", "major")).toStrictEqual([
			"C",
			"D",
			"E",
			"F",
...
describe("chordsFromScale: will return all the cords from a given scale", () => {
	it("Should return some chords", () => {
		expect(
			Notes.chordsFromScale(["C", "D", "E", "F", "G", "A", "B"])
		).toMatchSnapshot();
	});
});
