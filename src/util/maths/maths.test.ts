...
describe("randomWeights", () => {
	...
	it("should return the second value", () => {
		...
		const randomVal = Test.randomWeight(
			[
				{ percent: 1 / 2, value: 2 },
				{ percent: 1 / 3, value: 3 },
			],
...
describe("normalizeArray", () => {
	it("Should return a scaled down array", () => {
		const result = Test.normalizeArray(
			[0.5, 0.2, 0.7],
			(total, item) => {
				return (total += item);
			},
		...
		expect(result).toEqual({
			acc: 1.4,
			array: [0.35714285714285715, 0.14285714285714288, 0.5],
		});
	});
	it("Should return a scaled up array", () => {
		...
		expect(result).toEqual({
			acc: 0.7,
			array: [0.7142857142857143, 0.28571428571428575],
		});
	});
