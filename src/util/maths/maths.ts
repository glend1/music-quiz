...
export function arrayContainsArray<T>(
	array: Array<T>,
	wholeArray: Array<T>,
	fn = (i: T, j: T) => {
		return i == j;
...
export function randomWeight<T>(
	options: { percent: number; value: T }[],
	defaultValue: T
) {
...
export function normalizeArray<T>(
	array: T[],
	reduce: (total: number, item: T) => number,
	mapAndTotal: (item: T, total: number) => T
) {
	const arrCopy = JSON.parse(JSON.stringify(array));
	let acc = arrCopy.reduce(reduce, 0);
	return {
		acc,
		array: arrCopy.map((item: T) => {
