...
describe("midiEvents: These are the events used in midicontrols component", () => {
    describe("mouse: Should trigger when you press a mouse button", () => {
        it("press: Should push onto the array", () => {
            ...
            const {result: {current: {mouse}}} = renderHook(() => MidiEvents(array.current, true, 4))
            act(() => {
                mouse({target: {dataset: {natural: "D"} } } as any )
            })
            expect(array.current.array).toStrictEqual([{ note: "D4", name: "D", octave: 4, abc: "D", midi: 62 }])
        })
        it("depress: Should remove from the array", () => {
            const { result: array } = renderHook(() => useArray<IStdNote>())
            const result = renderHook(() => MidiEvents(array.current, true, 4))
            act(() => {
                result.result.current.mouse({target: {dataset: {natural: "C"} } } as any )
            });
            result.rerender()
            act(() => {
                result.result.current.mouse({target: {dataset: {natural: "D"} } } as any )
            });
            result.rerender()
            act(() => {
                result.result.current.mouse({target: {dataset: {natural: "C"} } } as any )
    ...
    describe("key: Should trigger when you press a button on the keyboard", () => {
        it("keydown: Should push onto the array", () => {
            const { result: array } = renderHook(() => useArray<IStdNote>())
            const {result: {current: {key}}} = renderHook(() => MidiEvents(array.current, true, 4))
            act(() => {
        ...
        it("keyup: Should remove from the array", () => {
            ...
            result.rerender()
            expect(array.current.array).toStrictEqual([{ note: "E4", name: "E", octave: 4, abc: "E", midi: 64 }])
        })
        ...
        describe("audioEvent: this controls how a tone is played", () => {
            it("keydown: Should play a tone",  () => {
                var i, d
                var mockCallback = jest.fn((str, data) => {i = str; d = data})
                const { result: array } = renderHook(() => useArray<IStdNote>())
                const {result: {current: {key}}} = renderHook(() => MidiEvents(array.current, true, 4, mockCallback))
                act(() => {
                    key({natural: "d"}, "keydown")
                })
                expect(i).toBe("start")
                expect(d).toStrictEqual({ note: 'D4', name: 'D', octave: 4, abc: 'D', midi: 62 })
            })
